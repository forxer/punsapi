...
class punsapi_sqlite
{
	...
	function punsapi_sqlite($db_host, $db_username, $db_password, $db_name, $db_prefix, $p_connect)
	{
	...
	function select($sql)
	{
		...
		if ($cur)
		{
			...
			while($res = $this->fetch_row($cur))
			{
				...
				for($j=0; $j<$num; $j++)
				{
					$arryRes[$i][strtolower(sqlite_field_name($cur, $j))] = $res[$j];		
				}
				$i++;
			}
